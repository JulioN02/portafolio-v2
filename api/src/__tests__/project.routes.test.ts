...
describe('Project routes (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    baseUrl = `http://127.0.0.1:${(server.address() as AddressInfo).port}/api/projects`;
  });
  ...
  describe('route order guard', () => {
    it('GET /tags resolves to the tags endpoint, NOT /:slug', async () => {
      (mockPrisma.project.findMany as jest.Mock).mockResolvedValue([
        ...
        { tags: ['proyecto-profesional'] },
      ]);

      const res = await fetch(`${baseUrl}/tags`);
      expect(res.status).toBe(200);
      const body = await res.json();
      expect(body).toEqual(['proyecto-profesional', 'proyecto-rapido']);
  ...
  describe('protected routes reject requests without JWT', () => {
    ...
    it('PATCH /:id/reorder → 401', async () => {
      const res = await fetch(`${baseUrl}/some-id/reorder`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  describe('public routes', () => {
    it('GET / returns the paginated project list', async () => {
      (mockPrisma.project.findMany as jest.Mock).mockResolvedValue([
